...
class Config:
    ...
    def __init__(self, mode):
        ...
        if self.mode == 'nb':
            ...
            self.dir = "transformer_results_mem_FINAL/"
            self.title = "Transformer"
            self.figure_name = "Transformer"
        elif self.mode == 'rf':
            self.dir = "./rf_results_cpu_FINAL_16_200/"
            self.title = "RF"
            self.figure_name = "RF"
        elif self.mode == 'arima':
            self.dir = "./arima_results_cpu_(1, 0, 0)/"
            self.title = "ARIMA"
    ...
    for timestamp in range(6):
        ...
        with open(config.dir + data_set + str(timestamp + 1) + ".txt", 'r') as file:
            for line in file:
                ...
                total_time.append(float(test_error_values[3]))
